...
program
  ...
  .option("--call-data-file <callDataFile>")
  .parse();
...
async function main() {
  try {
    ...
    const callData = cli.callData
      ? Binary.fromHex(cli.callData)
      : cli.callDataFile
      ? await getCallDataBytes(cli.callDataFile)
      : Binary.fromHex(
          await input({ message: "Hex-encoded call data (0x…)?" })
        );

    return tx.signAndSubmit(signingChoice, callData, endpoint);
  } catch (e) {
    console.error((e as Error).message);
  }
}

async function getCallDataBytes(path: string): Promise<Binary> {
  const file = await Deno.readFile(path);
  return Binary.fromBytes(file);
}
